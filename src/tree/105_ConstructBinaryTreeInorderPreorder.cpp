...
class Solution
{
  ...
  public:
								 ...
								 int start, int left, int right)
	{
		...
		int foundIndex =
			umap[root->val]; // searchIndex(inorder, left, right, root->val);
		root->left = recursiveBuildTree(preorder, inorder, start + 1, left,
										foundIndex - 1);
		root->right =
			recursiveBuildTree(preorder, inorder, start + foundIndex - left + 1,
							   foundIndex + 1, right);
		return root;
	}
