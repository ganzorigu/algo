...
class Solution
{
  public:
	int maxAreaOfIsland(vector<vector<int>> &grid)
	{
		...
		vector<vector<bool>> visitedGrid(grid.size(),
		...
		for (int i = 0; i < grid.size(); i++)
		{
			for (int j = 0; j < grid[0].size(); j++)
			{
				if (grid[i][j] == 1 && !visitedGrid[i][j])
				{
					...
					maxArea = max(maxArea,
								  recursiveIslandSize(grid, visitedGrid, i, j));
				}
			}
		}
		return maxArea;
	...
	int recursiveIslandSize(vector<vector<int>> &grid,
							vector<vector<bool>> &visitedGrid, int x, int y)
	{
